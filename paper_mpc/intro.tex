...
counterpart of the root bound of the KaPoCE branch-and-bound, and on large
graphs it contributes most of the bound; the block LPs add the rest
(Section~\ref{sec:exp-bounds} quantifies both).
...
held-out set fixed in advance, it certifies the best known solutions
within \numSnapGapLo{} to \numSnapGapHi{}, where scalable baselines certify
factors of \numBaseFacLo{} to \numBaseFacHi{}.  We compare with the sparse
star packing alone, the metric LP on the support with triangle or star rows,
and a cold-started LP, so that the contribution of each part is visible.
\end{enumerate}
