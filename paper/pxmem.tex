CertiFlip certifies its solutions, but on large graphs they remain
$0.1\%$--$0.4\%$ above those of KaPoCE (Section~\ref{sec:exp}).  This subsection
describes a second primal solver, \emph{PXMem}.  It rests on two elementary
exact facts: twins can be contracted, and two clusterings can be recombined
block by block without losing anything.  Neither fact is deep, and both have
close relatives in the literature, which we point out below; what we add is
their combination into a solver whose incremental formulas and
recombination are machine-checked (Section~\ref{sec:lean}).

\subsubsection{Critical-clique contraction}
...
\subsubsection{Partition crossover}
...
pairs between blocks separated in both.  The idea of decomposing two parents
into independent components and taking the better parent in each is that of
the partition crossover of \citet{WhitleyHH09gpx,WhitleyHH10hybrid} for tours,
of its generalisation GPX2~\citep{TinosWO20gpx2}, and of partition crossover
for pseudo-Boolean functions~\citep{TinosWC15px}; see also optimal
recombination~\citep{EremeevK14orp}.  Recombination through the overlap of the
parents' groups is also common in grouping genetic
algorithms~\citep{Falkenauer98grouping,GalinierH99coloring}, memetic graph
partitioning~\citep{SandersS12kaffpae,BenlicH11multilevel}, memetic
clustering~\citep{BiedermannHSS18memetic} and ensemble clustering with core
groups~\citep{OvelgonneG13ensemble}.  The argument is a few lines, and we
claim no novelty for it.  It combines \emph{any} two
clusterings, including clusterings produced by different solvers.  On our graphs, two independent runs typically differ in thousands of
blocks, most of them ties, and each run wins some of the others.

\subsubsection{Annealing moves}
...
\subsubsection{The memetic search}
