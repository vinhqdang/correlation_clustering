...
$0.1\%$--$0.4\%$ above those of KaPoCE (Section~\ref{sec:exp}).  This section
describes a second primal solver, \emph{PXMem}, whose solutions match or
beat KaPoCE on most of our graphs.  It rests on two exact facts: twins can be
contracted, and two clusterings can be recombined block by block without
losing anything.  Both results, the move formulas used by the implementation
and the guarantee of Theorem~\ref{thm:pxmem} are machine-checked
...
of twins are the \emph{critical cliques} of \citet{Guo09kernel}.
...
\end{lemma}
\begin{proof}
Let $A\ni u$ and $B\ni v$ be the two clusters and, for a cluster $X$, let
$f(X)$ be the number of mistakes between a vertex $z\in\{u,v\}$ and the
vertices of $X\setminus\{u,v\}$ if $z$ is placed in $X$; it is the same for $u$
and $v$ because they are twins.  Besides the cut positive pair $uv$, the
current cost involving $u$ or $v$ is $f(A)+f(B)$; after the first move it is
$2f(A)$, after the second $2f(B)$.  So $\delta_1+\delta_2=2f(A)+2f(B)
...
to an optimal clustering of $G^+$.  On the collaboration graphs the
...
Let $a$ and $b$ be clusterings and let $H$ be the bipartite graph whose nodes
...
$O(n+m)$ time.  The blocks are also the finest sets on which a block-wise
choice between the two parents always gives a clustering: any finer set would
split a cluster of $a$ or of $b$.  The operator is the analogue for clusterings
of the partition crossover of \citet{WhitleyHH09gpx} for tours.  It combines
\emph{any} two clusterings, including clusterings produced by different
solvers.  On our graphs, two independent runs typically differ in thousands of
...
replace a member only if they improve it.
