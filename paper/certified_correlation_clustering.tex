...
\begin{abstract}
Correlation clustering (equivalently, cluster editing) has approximation
algorithms with ratios below $1.5$, but the heuristics used in practice come
with no quality guarantee on large graphs: exact solvers stop at a few
hundred vertices, and the lower bounds that scale certify only a factor of
about two.  We attack this gap from both sides.
(i)~Since optimal clusters have diameter at most two, the metric LP can be
restricted to the $O(\sum_v\deg(v)^2)$ pairs at distance at most two without
losing validity or LP-rounding guarantees; we strengthen it with star and
local subgraph inequalities.
(ii)~Dual block-coordinate ascent turns this relaxation into an anytime lower
bound that is valid after every step.  Its certificates are checked in exact
arithmetic by a short checker that shares no code with the solver.
(iii)~The optimality gap decomposes exactly into non-negative local terms,
which yields local optimality certificates and guides a large-neighbourhood
search.  The resulting algorithm, CertiFlip, has expected cost at most
$3\,\OPT$ and returns a certified bound with every solution.  On the PACE~2021
exact track its bound proves optimality on 111 of 173 solved instances,
against 79 for the root bounds of the state-of-the-art branch-and-bound, and
improves the published lower bounds of two open instances.  On SNAP graphs
with up to $10^6$ edges it certifies the best known solutions within
$0.25\%$ to $10.1\%$, where scalable bounds certify factors of $1.17$ to $1.81$.
(iv)~Two clusterings can be recombined exactly, block by block of their
overlap.  The memetic solver PXMem built on this recombination keeps the
guarantee of $3$ in expectation.  In 150 runs of 600\,s on fifteen SNAP
graphs, run one after the other on the same core as the PACE~2021 winner
KaPoCE, it wins 67, ties 49 and loses 34.
(v)~The main combinatorial arguments are machine-checked in Lean~4.
\end{abstract}
...
hand}, in the sense of certifying algorithms~\citep{McConnellMNS11}.
Figure~\ref{fig:overview} shows how the parts fit together.  Our
contributions are as follows.

\begin{figure}[!tbp]
\centering
\resizebox{\textwidth}{!}{\input{figures/overview}}
\caption{Overview.  The primal side (blue) computes clusterings: Pivot and
iterated flipping give a start with $\mathbb E[\cost]\le3\,\OPT$, and either
the gap-guided LNS of CertiFlip or the memetic search PXMem improves it.  The
dual side (orange) builds the distance-two support and a lower bound by block
dual ascent.  The gap decomposition splits $\cost(\mathcal C)-\LB$ into
non-negative local terms and tells the LNS where to search.  The dual
...
\item \textbf{CertiFlip} (Section~\ref{sec:algo}).  We implement a heuristic
version of the iterated flipping local search of \citet{CohenAddadLPTYZ24},
with cluster-insertion moves generated by neighbourhood cleaning and
multilevel refinement.  It explores only a polynomial family of insertions and
does not inherit the guarantees of that algorithm.  Together
...
the guarantee of $3$ in expectation (Theorem~\ref{thm:pxmem}).  Against KaPoCE,
run one after the other on the same core with 600\,s each, it is better on
four of fifteen SNAP graphs and worse on one by the sign test (one each after
Holm correction), and equal or indistinguishable on the others.

\item \textbf{Machine-checked proofs and checked certificates}
(Section~\ref{sec:lean}).  The main combinatorial lemmas, the incremental
cost formulas of the implementation, the validity of the dual bound and the
crossover theorem are proved in Lean~4 with Mathlib; Table~\ref{tab:lean}
lists what is and is not covered.  Every lower bound reported in the
experiments is recomputed from its exported certificate by an independent
checker in exact arithmetic.
...
solutions within $0.25\%$ to $10.1\%$ of optimal.  Previously scalable bounds
...
separated in every optimal solution.  Twin (critical-clique) reductions and
their weighted variants are standard in the parameterised
algorithms~\citep{Guo09kernel,ProttiDS09modular,BockerBBT09weighted,
Bocker12golden,ChenM12kernel} and in practical tools such as FORCE and
TransClust~\citep{WittkopBLR07force,WittkopEtAl10transclust}.  Cutting-plane
methods for the clique partitioning polytope go back to
\citet{GrotschelW89,GrotschelW90facets}, with recent work on facets and
redundancy~\citep{irmai2024chorded,koshimura2022concise}.
\citet{MiyauchiS15redundant} and \citet{miyauchi2018exact} show that many
triangle inequalities of the clique partitioning formulation are redundant;
the distance-two relaxation is a different reduction, which removes
variables rather than rows.  The LP relaxations of \citet{veldt2022stc}, based
on strong triadic closure, are also supported on the open wedges of $G^+$;
they yield fast approximation algorithms and bounds within a factor of about
two, while our relaxation keeps the metric LP on $P$.  For
...
include vertex moves~\citep{elsner2009bounding}, heuristics for clique
partitioning~\citep{BruscoK09clique,ZhouHG16threephase,LuZH22hybrid}, local
search for multicut~\citep{levinkov2017comparative}, Louvain and Leiden with the
...
\section{Certified lower bounds}\label{sec:bounds}

This section develops the lower-bound side of Figure~\ref{fig:overview}: a
relaxation supported on the pairs at distance at most two
(Section~\ref{sec:relax}), an anytime dual bound on it
(Section~\ref{sec:dual}), and an exact decomposition of the optimality gap
that localises where a clustering can still improve (Section~\ref{sec:gap}).
...
\subsection{Gap decomposition and local certificates}\label{sec:gap}
...
We now describe the two solvers of Figure~\ref{fig:overview}.  CertiFlip
(Section~\ref{sec:algo}) couples a flipping local search with the bound of
Section~\ref{sec:bounds} and returns a certificate with every solution.
PXMem (Section~\ref{sec:pxmem}) is a memetic solver built on an exact
recombination of clusterings; it aims at the best primal value.
Section~\ref{sec:lean} states what of both is machine-checked and how the
reported bounds are verified.
...
\subsection{Exact recombination: the PXMem solver}\label{sec:pxmem}
\input{pxmem}

\subsection{Machine-checked proofs and certificate checking}\label{sec:lean}
\input{verification}
...
They also give a primal method that knows where to search.  Our heuristic
version of the iterated flipping local search of \citet{CohenAddadLPTYZ24}
comes within a few tenths of a percent of the best engineered heuristic in a
fraction of its running time.  The exact
partition crossover closes the remaining gap on most graphs: PXMem keeps the
a-priori guarantee, is not significantly worse than KaPoCE on fourteen of fifteen graphs at
equal budget on the same core, and turns additional cores into better
solutions.  The main combinatorial arguments are machine-checked, and every
reported lower bound is confirmed by an independent exact checker.

\paragraph{Limitations.}
\begin{itemize}
\item KaPoCE remains better than PXMem on \texttt{com-Amazon} (by $0.003\%$ at
\SI{600}{s}) and, not significantly, on \texttt{com-Youtube}.  At short
budgets of \SI{60}{s} and \SI{150}{s} it is better overall, most clearly on
the six largest graphs (median $0.002\%$ to $0.64\%$), where our initial
population takes too much of the budget.  The same holds at
\SI{600}{s} on the four PACE heuristic-track instances with more than
$10^6$ vertices ($0.02\%$ to $0.32\%$).  PXMem also needs up to twice the memory of KaPoCE.
\item The formal development covers the statements listed in
Table~\ref{tab:lean}, not all results of the paper, and it covers the
mathematics, not the code.  The Pivot guarantee used in
Theorems~\ref{thm:certiflip} and~\ref{thm:pxmem} enters it as a hypothesis.
