...
gaps of $0.54\%$ and $0.45\%$.  CertiFlip reaches 192 instances, and its
largest gap is $0.45\%$.  KaPoCE reaches all 200 instances.  This is not
...
cost 2788 within the budget.  We verified the CertiFlip solution
independently, and it is included in the repository.
...
The CertiFlip bound (Theorem~\ref{thm:anytime}), computed within half of the
...
at most 30.  Our bound exceeds the star-packing bound on 59 instances, is
lower on 43 and equal on 71.  The comparison depends on density.  On the 35
...
instances the star packing is better ($99.5\%$ to $99.7\%$ against
$98.0\%$ to $98.9\%$).  There the block LPs are too large to converge within the budget,
and the bound falls back to the best packing.

\begin{table}[t]
\centering
\caption{The 27 open PACE 2021 exact instances: published upper bound (UB)
and star-packing lower bound (LB)~\citep{BlasiusEtAl22sea}, and our best
solution and bound from one 60\,s CertiFlip run.  Bold marks improvements
...
published lower bound on \texttt{exact179} (from 620 to 632, so the optimum is
now known to be 632 or 633) and on \texttt{exact180} (from 1052 to 1069).  On
