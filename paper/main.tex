...
Distance-Two Relaxations, Anytime Dual Bounds,\\ and Gap-Guided Local Search}
...
solvers stop at a few hundred vertices, the metric LP has $\Theta(n^3)$
constraints, and the scalable lower bounds known so far certify only a factor
of about two.  We close this gap from both sides.  (i)~Using the fact that
optimal clusters have diameter at most two, we show that the metric LP can be
...
strengthen it with star and local subgraph inequalities that close most of the
remaining integrality gap on real graphs.  (ii)~We give an anytime,
always-valid lower bound computed by dual block-coordinate ascent in which
every block is an exact LP; it needs only one number per pair and runs on graphs
with millions of pairs.  (iii)~We prove an exact decomposition of the
optimality gap into non-negative per-pair and per-constraint terms, which
yields a local optimality certificate and guides a large-neighbourhood search
with exact sub-MIPs.  Combined with the first implementation of the iterated
flipping local search of Cohen-Addad et al., these ingredients form
\emph{CertiFlip}, an algorithm with an a-priori guarantee ($3$ in expectation,
and $2.06$ whenever the relaxation is solved to optimality) that outputs a
certified bound on its own approximation ratio.  On the PACE~2021 benchmark
and on fifteen SNAP graphs with up to $3\cdot 10^6$ edges, CertiFlip matches
the solution quality of the PACE~2021 winner and certifies it within a few
percent of optimal where previously no certificate below a factor of about two
was available.
\end{abstract}
...
The scalable combinatorial lower bounds certify solutions only within a factor
of about two~\citep{veldt2022stc}.  The PACE report~\citep{KellerhalsKNZ21pace}
...
these raise the bound substantially: on \texttt{ca-GrQc} the gap to the best
known solution shrinks from $23\%$ to $0.5\%$.
...
multipliers (Theorem~\ref{thm:anytime}).  Blocks come from randomised graph
partitions and from regions where the current optimality gap is large.
...
We release an open-source pipeline with thirteen solvers and bounds.  We run
it on the 200 PACE~2021 exact-track instances, whose optima are mostly known,
and on fifteen SNAP graphs with up to $1.1\cdot 10^6$ vertices.
\end{enumerate}
...
\State $y\gets\textsc{BlockAscent}(P,\mathcal C,T/2)$ \Comment{METIS sweeps, then gap-guided blocks}
...
\State \Return $\mathcal C$ and $\lceil\LB(y)\rceil$
\end{algorithmic}
\end{algorithm}
...
\item[(c)] If line~6 is executed, $\mathbb E[\cost(\mathcal C)]\le 2.06\,\OPT$.
\end{enumerate}
\end{theorem}
\begin{proof}
...
quality on real instances had not kept pace.  The distance-two relaxation,
the anytime block-dual bound and the gap decomposition make the LP machinery
usable at scale and give a primal heuristic that knows where to search.
Open directions include parallel block solves, cluster-LP columns inside
blocks, and replacing METIS partitions by overlapping blocks with Lagrangian
cost splitting.
