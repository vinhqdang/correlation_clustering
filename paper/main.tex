...
approximation ratio.  On the PACE~2021 exact track it finds a new best
solution for an open instance and proves optimality on more instances than
the root bounds of the state-of-the-art branch-and-bound.  On SNAP graphs its
bounds certify the solutions of the PACE~2021 winner within about one percent
of optimal, on graphs far beyond the reach of dense-matrix bounds.
\end{abstract}
...
triangles is a feasible starting point.  We compute one on the sparse support
with a ruin-and-recreate local search: on \texttt{ca-HepTh} it exceeds the
root bound of the KaPoCE branch-and-bound in less than half the time and
with $O(|P|)$ instead of $\Theta(n^2)$ memory.  Blocks come from randomised
